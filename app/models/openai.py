...
class Model(BaseModel):
    ...
    created: Optional[int] = Field(
        None, description="Unix timestamp (for OpenAI compatibility)"
    )
    owned_by: Optional[str] = Field(
        default="anthropic", description="Organization that owns the model"
    )
    object: Optional[Literal["model"]] = Field(
        default="model", description="Object type (for OpenAI compatibility)"
    )
...
class ModelList(BaseModel):
    ...
    object: Optional[Literal["list"]] = Field(
        default="list", description="Object type (for OpenAI compatibility)"
    )
